...
from dbConnect import JWT_SECRET_KEY
...
def validate_user(email,password):
  current_user=db.read(""" SELECT * FROM users WHERE email = %s""",(email,))
  if len(current_user)==1:
    saved_password_hash=current_user[0]["password_hash"]
    saved_password_salt=current_user[0]["password_salt"]

    password_hash=generate_hash(password,saved_password_hash)
